...
In this section, we conduct two sets of experiments. In the first part, we provide empirical support for our optimality theory in Section~\ref{sec:confidence} by evaluating, on synthetic and real text generation tasks, the local likelihood optimization objective \eqref{eq:trainable_confidence} attained by different unmasking strategies. In the second part, we train unmasking policies with \alg{} and evaluate the resulting sampling performance on text, code, and math benchmarks across multiple MDMs. In addition to the main experiments, we also study the effect of KL regularization, temperature, and the per-step unmasking schedule.

\subsection{Numerical Analysis on Likelihood Optimization}\label{subsec:numerical-analysis}

\paragraph{Landscape Visualization under Synthetic Distribution.}

We consider the task of unmasking 2 tokens from a length-3 sequence. There are exactly three possible unmasking actions: unmasking indices $\{0,1\}$, $\{0,2\}$, or $\{1,2\}$, enabling a full visualization of the optimization landscape in 3-dimensional space. To simulate MDM predictions $p_t^\theta(\cdot \mid \bm x_t)$, we use a synthetic distribution described in Appendix~\ref{subsec:synthetic}. In Figure~\ref{fig:opt-intro}, we plot the landscape and each unmasking method on the policy simplex. The exact optimizer of \eqref{eq:trainable_confidence} ($\bigstar$) is close to the entropy-based greedy point ($\blacktriangle$), with closely matching objective values, illustrating Theorem~\ref{thm:confidence} and Theorem~\ref{thm:confidence-rand}. We provide additional results in Appendix~\ref{subsec:synthetic}. 

\paragraph{Objective Values in Real Text Generation.} 

We also evaluate the local objective value \eqref{eq:trainable_confidence} attained by different unmasking strategies on text generation with RADD~\citep{ou2024your}, a pretrained MDM. We compare uniform unmasking, greedy confidence unmasking based on the top-2 probability margin, and its randomized counterpart. At each step, the objective \eqref{eq:trainable_confidence} is approximated by Monte Carlo estimation. In Figure~\ref{fig:radd-obj-val}, we set the generation length to 1024 and use 512 diffusion sampling steps. We observe that both confidence methods consistently achieve higher objective values than the uniform method, in alignment with our theory in Section~\ref{sec:confidence}. The objective value of greedy confidence method is initially the highest but would collapse at the final sampling steps, suggesting the importance of randomization in practice which can also be observed in Section~\ref{subsec:vlp-eval}. See Appendix~\ref{subsec:radd-gen-exp} for further results.

\subsection{Evaluation of VLP}\label{subsec:vlp-eval}
...
We first ablate the \textbf{KL regularization strength} $\lambda$ in RADD text generation experiments in Table~\ref{tab:text-kl}. The effect of $\lambda$ reveals that KL regularization consistently brings performance improvement to generation quality across multiple sampling budgets. We also study the effect of choosing different \textbf{diffusion steps} $K$ in VLP training. In RADD text generation experiments in Table~\ref{tab:text-step}, training with $K=32$ gives the strongest overall performance, achieving the best perplexity at most evaluation steps. The results also suggest that VLP trained on a fixed $K$ is able to work on a broader range of different evaluation steps. Finally, Table~\ref{tab:temp-humaneval} reports the performance of VLP under different choices of \textbf{sampling temperature} through evaluations. Appendix~\ref{sec:vlp-additional-results} provides further details on these studies. 
