...
\begin{figure}[h]
    ...
    \caption{\textbf{(a)}: The landscape of the local likelihood maximization problem \eqref{eq:trainable_confidence} over the policy simplex, and different unmasking methods with their attained objective values. \textbf{(b)}: Monte Carlo estimation of per-step local objective values \eqref{eq:trainable_confidence} attained by different unmasking methods in text generation with RADD~\citep{ou2025absorbingdiscretediffusionsecretly}. \textbf{(c)}: Evaluation results of VLP compared to uniform and confidence unmasking methods in text generation with RADD, and code and math generation with Dream-7B~\citep{ye2025dream7bdiffusionlarge}. Data is taken from Table~\ref{tab:text-gen} and~\ref{tab:dream-all} and rescaled by the best and worst performance.}
\end{figure}
...
We evaluate \alg{} on code and math generation using two 7B-scale MDMs: Dream-7B~\citep{ye2025dream7bdiffusionlarge} and DiffuCoder-7B~\citep{gong2025diffucoderunderstandingimprovingmasked}. We train the policy with OpenCoder-SFT~\citep{Huang2024OpenCoderTO} and MetaMathQA~\citep{yu2023metamath} dataset, and evaluate on two code generation benchmarks: HumanEval~\citep{chen2021evaluatinglargelanguagemodels} as well as MBPP~\citep{austin2021programsynthesislargelanguage}, and two math generation benchmarks: GSM8K~\citep{cobbe2021training} and MATH500~\citep{lightman2023let}. The detailed setup is provided in Appendix~\ref{sec:vlp}.
...
We first ablate the \textbf{KL regularization strength} $\lambda$ in Table~\ref{tab:text-kl}. Compared to the baseline case of no regularization ($\lambda=0$), setting a nonzero $\lambda$ generally improves perplexity at most cases of sampling steps. We also study the effect of choosing different \textbf{diffusion steps} $K$ in VLP training. In Table~\ref{tab:text-step}, training with $K=32$ gives the strongest overall performance, achieving the best perplexity at most evaluation steps. The results also suggest that VLP trained on a fixed $K$ is able to work on a broader range of different evaluation steps. Table~\ref{tab:temp-humaneval} reports the performance of VLP under different choices of \textbf{sampling temperature}. Appendix~\ref{sec:vlp-additional-results} provides further details on these studies. 
