...
\end{cases}
\end{equation*}
with a state space $\mathcal V\cup\{\mask\}$. Letting $\bar\sigma(t)=\int_0^t\sigma(s)\,ds$ and $\alpha_t = e^{-\bar\sigma(t)}$, the distribution of a token at time $t$ conditioned on the initial state $x$ is given by
\begin{equation*}
\label{eq:forward_seq}
    q_{t|0}(y|x)
=
%e^{-\bar\sigma(t)}
\alpha_t\mathbf 1[y=x]
+
\bigl(1-
%e^{-\bar\sigma(t)}
\alpha_t\bigr)\mathbf 1[y=\mask].
\end{equation*}
