...
We first state the complete version of Theorem~\ref{thm:confidence-rand} on a general non-deterministic policy.
\begin{theorem}
    \label{thm:confidence-complete}
     Consider a policy $\pi$ that selects an index set of size at least $N$. 
     If (1) $\pi$ is a deterministic policy, or (2) there exists $\mathcal U$ such that for all $\mathcal U'$, $p_{t_k}^\theta$ satisfies
     \begin{align*}
     ...
     \end{align*}
    , then the optimal strategy $\pi^*$ to \eqref{eq:trainable_confidence} satisfies $\pi^*(\cdot|\bm x_k)=\mathbf1_{\mathcal U^*}$, where
...
More concretely, let $N=L/K$ be the number of positions to unmask at one step. At inference time, we sample $N$ distinct positions using \texttt{torch.multinomial} with \texttt{replacement=False}. Thus, for a size-$N$ set $\mathcal U$, the induced set probability is the sequential without-replacement probability
\begin{align*}
    \pi_{\mathrm{norep}}(\mathcal U|\bm x_k)
    =
    \sum_{\sigma\in\mathrm{Sym}(\mathcal U)}
    \prod_{r=1}^{N}
    \frac{p_{\sigma_r}}{1-\sum_{s<r}p_{\sigma_s}},
\end{align*}
where the sum is over all orderings of the elements in $\mathcal U$. During policy training, however, directly evaluating $\pi_{\mathrm{norep}}$ is computationally prohibitive and we use the surrogate \texttt{torch.multinomial} with \texttt{replacement=True}. This yields the simple likelihood for an ordered draw $(i_1,\ldots,i_N)$,
\begin{align*}
    \pi_{\mathrm{rep}}(i_1,\ldots,i_N|\bm x_k)=\prod_{r=1}^{N}p_{i_r},
\end{align*}
which makes the policy log-probability straightforward to compute by summing $\log p_{i_r}$. This surrogate can be bounded from both sides by the following. For a distinct set $\mathcal U$, the with-replacement probability of drawing exactly the elements of $\mathcal U$ once, in any order, is $N!\prod_{i\in\mathcal U}p_i$. Since every normalizing denominator in the without-replacement probability is at most one,
\begin{align*}
    N!\prod_{i\in\mathcal U}p_i
    \leq
    \pi_{\mathrm{norep}}(\mathcal U|\bm x_k).
\end{align*}
Moreover, if $p_{\mathcal U}:=\sum_{i\in\mathcal U}p_i<1$, then each nontrivial denominator is at least $1-p_{\mathcal U}$, giving the upper bound
\begin{align*}
    \pi_{\mathrm{norep}}(\mathcal U|\bm x_k)
    \leq
    \frac{N!\prod_{i\in\mathcal U}p_i}{(1-p_{\mathcal U})^{N-1}}.
\end{align*}
...
For evaluation, we use zero-shot setting (no in-context demonstrations) and prompts for HumanEval, MBPP, and GSM8K are formatted with the chat template (Table~\ref{tab:chat-humaneval}, \ref{tab:chat-mbpp},\ref{tab:chat-gsm8k}). All prompts are padded with mask tokens up to the requested generation length. We use context length 1024 for code tasks (HumanEval, MBPP) and 512 for math tasks (GSM8K, MATH500). Generation uses the diffusion sampler for the specified number of steps. For confidence and policy variants, an algorithm temperature controls whether positions are selected greedily or sampled from a softmax over scores. Unless otherwise specified, the temperature for performing categorical sampling among the vocabulary is set to $0$.



\subsubsection{Additional Results}\label{sec:vlp-additional-results}

\paragraph{RADD} Table~\ref{tab:text-gen-full} evaluates the mean and standard deviation of GPT-2 perplexity as well as sample entropy in unconditional generations from RADD.

\begin{table*}[h]
\centering
\caption{Mean GPT-2 perplexity ($\downarrow$), perplexity standard deviation, and sample entropy of unconditional generations from RADD under different unmasking strategies and numbers of sampling steps. All generations have context length 1024. OpenWebText dataset has sample entropy 5.44.}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{cccccccccccccccc}
\toprule
& \multicolumn{5}{c}{Perplexity mean}
& \multicolumn{5}{c}{Perplexity std. dev.}
& \multicolumn{5}{c}{Entropy} \\
...
\midrule
uniform
& 330.3 & 192.1 & 145.5 & 134.5 & 116.8
& 100.6 & 54.5 & 47.4 & 46.7 & 60.0
& 5.77 & 5.71 & 5.68 & 5.69 & 5.64 \\
confidence-g
& 1123.0 & 1036.5 & 982.4 & 860.7 & 550.8
& 290.8 & 333.5 & 330.6 & 335.4 & 257.0
& 5.72 & 5.64 & 5.59 & 5.60 & 5.52 \\
confidence-r
& 313.0 & 181.6 & 140.7 & \textbf{116.7} & \textbf{103.4}
& 78.2 & 53.3 & 50.5 & 41.1 & 35.3
& 5.74 & 5.67 & 5.63 & 5.61 & 5.59 \\
\midrule
\alg{}
& \textbf{300.8}
& \textbf{174.8}
...
& 128.9
& 106.7
& 77.4 & 43.7 & 38.1 & 45.4 & 47.8
& 5.78 & 5.72 & 5.67 & 5.67 & 5.63 \\
\bottomrule
\end{tabular}%
}
\label{tab:text-gen-full}
\end{table*}


\paragraph{Evaluation on DiffuCoder-7B.} Table~\ref{tab:diffucoder-all} reports the corresponding full DiffuCoder-7B results on HumanEval, MBPP, GSM8K, and MATH500. VLP consistently improves over uniform unmasking and is able to achieve matching or better performance compared to confidence methods. Note that the backbone DiffuCoder-7B model is finetuned specifically on coding performance~\citep{gong2025diffucoderunderstandingimprovingmasked}, leading to a degrade of math abilities regardless of decoding methods.
...
\begin{table*}[h]
\centering
\caption{Effect of KL regularization strength $\lambda$ in VLP training for RADD. We report mean GPT-2 perplexity ($\downarrow$), perplexity standard deviation, and entropy of unconditional generation at different sampling steps. OpenWebText dataset has sample entropy 5.44.}
\setlength{\tabcolsep}{4.0pt}
\begin{tabular}{ccccccccccccc}
\toprule
& \multicolumn{4}{c}{Perplexity mean}
& \multicolumn{4}{c}{Perplexity std. dev.}
...
\paragraph{Effect of sampling steps $K$ in VLP training.} Table~\ref{tab:text-step} ablates the choice of the training diffusion step $K$ for RADD, with $\lambda$ fixed at $10^{-2}$. Although each policy is trained with a fixed $K$, we evaluate it across several sampling budgets. Training with $K=32$ gives the strongest overall perplexity, suggesting that a mid-range training horizon transfers well across different numbers of inference steps.

\begin{table*}[h]
\centering
\caption{Effect of choice of $K$ in VLP training for RADD. We report mean GPT-2 perplexity ($\downarrow$), perplexity standard deviation, and entropy of unconditional generation at different sampling steps. OpenWebText dataset has sample entropy 5.44.}
\setlength{\tabcolsep}{4.0pt}
\begin{tabular}{ccccccccccccc}
\toprule
& \multicolumn{4}{c}{Perplexity mean}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\caption{Chat template for VLP GSM8K evaluation.}
\begin{chatbox}
    <|im\_start|>system\\
    You are a helpful assistant.<|im\_end|>\\
    <|im\_start|>user\\
...
\end{chatbox}
\label{tab:chat-gsm8k}
\end{minipage}
\end{table*}


\subsubsection{Datasets, Models, and Licenses}\label{sec:asset-licenses}

Table~\ref{tab:asset-licenses} lists the public datasets and pretrained model checkpoints used in our experiments, together with their public URLs and license information. License names are taken from the corresponding Hugging Face model or dataset cards when available.

\begin{table*}[h]
\centering
\caption{Public datasets and pretrained models used in the experiments.}
\setlength{\tabcolsep}{4pt}
\small
\begin{tabularx}{\textwidth}{p{0.18\textwidth}p{0.16\textwidth}X p{0.22\textwidth}}
\toprule
Asset & Type & URL & License \\
\midrule
OpenWebText & Dataset & \url{https://huggingface.co/datasets/Skylion007/openwebtext} & CC0-1.0 \\
OpenCoder-SFT stage 2 & Dataset & \url{https://huggingface.co/datasets/OpenCoder-LLM/opc-sft-stage2} & MIT \\
MetaMathQA & Dataset & \url{https://huggingface.co/datasets/meta-math/MetaMathQA} & MIT \\
RADD & Model & \url{https://huggingface.co/JingyangOu/radd-lambda-dce} & Apache-2.0 for official code release; no license specified on the Hugging Face model card \\
Dream-7B-Base & Model & \url{https://huggingface.co/Dream-org/Dream-v0-Base-7B} & Apache-2.0 \\
DiffuCoder-7B-Instruct & Model & \url{https://huggingface.co/apple/DiffuCoder-7B-Instruct} & Apple Machine Learning Research License (\texttt{apple-amlr}) \\
\bottomrule
\end{tabularx}
