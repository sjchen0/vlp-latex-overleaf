...
\begin{algorithm}[tb]
  ...
  \begin{algorithmic}[1]
    ...
    \Repeat
      ...
      \State $g_{\mathrm{REINFORCE}} \gets
      \frac{1}{B}\sum_{i=1}^B
      \left[
      \nabla_\phi s^{(i)}
      +
      ...
      \nabla_\phi \log w^{(i)}
      \right]$
      \State $\phi \gets \phi - \eta\, g_{\mathrm{REINFORCE}}$
    \Until{converged}
  \end{algorithmic}
\end{algorithm}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/vlp.png}
    \caption{An illustration of the shared-torso architecture of \alg{} with a trained MDM.}
    \label{fig:vlp}
\end{figure}


\section{Further Experiments Details}\label{sec:experiment-appendix}
...
\subsubsection{Experiment Setup}\label{sec:code-math-details}

\paragraph{Text Generation Details.}

For the RADD text generation experiments, we use the pretrained \texttt{JingyangOu/radd-t-dce} checkpoint, corresponding to the small RADD configuration with 12 Transformer blocks, hidden size 768, 12 attention heads, sequence length 1024, and approximately 162M parameters. The backbone is loaded from Hugging Face and kept frozen throughout policy training. The policy network follows Section~\ref{subsec:architecture}: it takes the frozen RADD hidden states together with binary indicators for masked and unmasked positions, applies a one-layer RoPE Transformer policy module, and outputs forward/backward unmasking distributions over sequence positions. The policy has approximately 5.3M trainable parameters, about 3\% of the RADD backbone.

We train the policy on OpenWebText. Text is tokenized with the GPT-2 tokenizer, appended with EOS tokens, concatenated, and split into fixed-length chunks of 1024 tokens. Since this is unconditional generation, there is no prompt portion and all positions are eligible for masking or unmasking. We use Wikitext-103 as the validation set. Unless otherwise specified, policy training uses batch size 4, one gradient accumulation step, and \(K=64\) discrete masking steps for the RADD runs used in our reported sampling experiments. The discrete time grid consists of \(K\) points in \((0,1)\), transformed by exponent 0.9. We optimize with AdamW using learning rate \(3\times 10^{-4}\), weight decay 0, \((\beta_1,\beta_2)=(0.9,0.999)\), \(\epsilon=10^{-8}\), 2500 warmup steps, gradient clipping at norm 1.0, and EMA decay 0.9999 for the policy parameters. Training is run on a single A100 80GB GPU.

During policy training, each batch is used to sample bidirectional masking trajectories. At each discrete step, the forward policy samples a subset of currently unmasked tokens to mask, the frozen RADD model evaluates the conditional token likelihood of reconstructing the removed tokens, and the backward policy scores the corresponding unmasking distribution. The loss combines the frozen-model token likelihood with the learned forward and backward policy probabilities, using a REINFORCE-style correction for the sampled forward subset. We additionally include the KL/entropy regularization term described in Section~\ref{subsec:architecture}, with the regularization weight varied in the KL ablation.

For evaluation, we generate unconditional sequences of length 1024 using the RADD diffusion sampler. We evaluate sampling budgets from 16 to 256 steps. For each setting, we generate 100 independent samples and report the mean and standard deviation of GPT-2-large perplexity. The evaluation model is \texttt{gpt2-large}; perplexity is computed from the average next-token cross entropy of the generated sequence. We compare uniform unmasking, greedy margin-based confidence unmasking, randomized margin-based confidence unmasking, and the learned policy. In the learned-policy sampler, the policy distribution is temperature-scaled, truncated to top-\(p=0.99\), and sampled without replacement to choose the positions revealed at each step; token values are then sampled from the frozen RADD conditional distribution.


\paragraph{Code and Math Generation Details.}

For both 7B models, we evaluate HumanEval~\citep{chen2021evaluatinglargelanguagemodels}, MBPP~\citep{austin2021programsynthesislargelanguage}, GSM8K, and MATH500. HumanEval and MBPP are evaluated using Pass@1 ($\uparrow$), while GSM8K and MATH500 are evaluated using final-answer accuracy ($\uparrow$). All evaluations are run through the LM Evaluation Harness with our DiffuCoder wrapper.

The policy is trained on instruction-output pairs from OpenCoder-SFT and MetaMathQA. For code training, we use the \texttt{educational\_instruct} split of \texttt{OpenCoder-LLM/opc-sft-stage2}. For math training, we use \texttt{meta-math/MetaMathQA}. Each example is converted to the chat format shown in Table~\ref{tab:chat-sft}. We tokenize each formatted sequence with the corresponding model tokenizer, truncate or pad to length 1024, and construct a prompt mask marking all tokens before the assistant response. During training and inference, prompt tokens are kept fixed; only response tokens are masked, unmasked, or generated.
...
\begin{table}[t]
\centering
\caption{Effect of KL regularization strength $\lambda$ in policy training for RADD. We report the perplexity ($\downarrow$) of unconditional generation at different sampling steps.}
\begin{tabular}{cccccc}
\toprule
$\lambda$ & 16 & 32 & 64 & 128 & 256 \\
\midrule
0    & 351.2 & 198.0 & 152.6 & 122.2 & 115.4 \\
$10^{-2}$ & 332.2 & 197.0 & 149.2 & 129.0 & 122.3 \\
$10^{-1}$ & \textbf{321.2} & \textbf{183.0} & \textbf{128.7} & \textbf{119.3} & \textbf{114.5} \\
\bottomrule
\end{tabular}
\label{tab:text-kl}
\end{table}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[h]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figs/vocab_temp_humaneval.png}
        \caption{Vocabulary temperature.}
    \end{subfigure}
    \hfill
    \begin{subfigure}[h]{0.48\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figs/policy_temp_humaneval.png}
        \caption{Policy temperature.}
    \end{subfigure}
    \caption{Temperature analysis on HumanEval.}
    \label{fig:temp-humaneval}
\end{figure}
...
\begin{table}[h]
    ...
    \begin{chatbox}
        ...
        \{output\}\\
        <|im\_end|>
    \end{chatbox}\label{tab:chat-sft}
\end{table}
...
\begin{table}[h]
    ...
    \begin{chatbox}
        ...
        ```python
    \end{chatbox}\label{tab:chat-humaneval}
\end{table}
...
\begin{table}[h]
    ...
    \begin{chatbox}
        ...
        Here is the code to solve this problem:\\
        ```python
    \end{chatbox}\label{tab:chat-mbpp}
\end{table}
\subsubsection{Generation Examples}