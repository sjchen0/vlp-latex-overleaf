...
\jiaxin{The Maximum Likelihood Policy}}
Recall that a major question motivating this work is how we can theoretically compare different unmasking policies and search for the best one. This is made possible by connecting our discrete-time sampling process $p_k^{\theta,\pi}$ to \jiaxin{maximum likelihood}, 
...
\begin{theorem}
    \label{thm:confidence}
     ...
     \end{align*}
     for some $\mathcal U$ and arbitrary $\mathcal U'$ \jiaxin{Do you mean there exists some $\mathcal{U}$ such that for any $\mathcal{U}'$? Also, I find this condition hard to interpret - we can improve the readability by simplifying the theorem as stating "the optimal deterministic policy is entropy minimized" and defer this extension to a remark that follows.}, then the optimal strategy $\pi^*$ to \eqref{eq:trainable_confidence} satisfies $\pi^*(\cdot|\bm x_k)=\mathbf1_{\mathcal U^*}$, where
...
\jiaxin{I feel the D-sup condition used here is too unrealistic. This section may not add too much value in my opinion. Shall we remove it?}
