...
\subsection{The Maximum Likelihood Policy}
Recall that a major question motivating this work is how we can theoretically compare different unmasking policies and search for the best one. This is made possible by connecting our discrete-time sampling process $p_k^{\theta,\pi}$ to maximum likelihood, 
% likelihood optimization, 
a fundamental objective of generative modeling. Specifically, since $p_k^{\theta,\pi}$ is intended to recover the data distribution at the endpoint $k=0$, its generative performance can be measured by the sample log-likelihood under the training data distribution $q_0$. Under this criterion, the optimal policy is obtained by
...
\begin{theorem}[Informal]
    \label{thm:confidence}
     Consider a policy $\pi$ that selects an index set of size at least $N$. Then, the optimal deterministic policy $\pi^*$ to \eqref{eq:trainable_confidence} satisfies $\pi^*(\cdot|\bm x_k)=\mathbf1_{\mathcal U^*}$, where
     $$\mathcal U^*={\arg\max}_{|\mathcal U|=N}\sum_{i\in\mathcal U}-H_k(i).$$
\end{theorem}

\jiaxin{Do you mean there exists some $\mathcal{U}$ such that for any $\mathcal{U}'$? Also, I find this condition hard to interpret - we can improve the readability by simplifying the theorem as stating "the optimal deterministic policy is entropy minimized" and defer this extension to a remark that follows.} \sijin{I prepared a simple statement in main text, and move the condition to appendix}

In words, $\pi^*$ will deterministically output the index set $\mathcal U^*$ with the smallest total entropy (highest confidence) for the next step's unmasking. Indeed, the optimality result generalizes to a certain subset of non-deterministic policies, and we provide a full statement and proof in Appendix~\ref{subsec:proof-confidence}. We also leave remarks on the connection to prior entropy-based criteria and extensions to constrained confidence samplers in Appendix~\ref{subsec:proof-confidence}.
...
\jiaxin{I feel the D-sup condition used here is too unrealistic. This section may not add too much value in my opinion. Shall we remove it?} \sijin{I think this section tries to theoretically understand the distribution shift from q to p, which is an important question. We only obtained result with Renyi-divergence at $\infty$, which is surely unrealistic and we didn't succeed in deriving a KL-based result. @Jason @Chang what do you think?}
