...
\begin{figure}[t]
    ...
    \begin{subfigure}[t]{0.3\linewidth}
        ...
        \begin{minipage}[c][0.17\textheight][c]{\linewidth}
            \centering
            \includegraphics[height=0.17\textheight,width=\linewidth,keepaspectratio]{figs/policy_simplex_seed_5.png}
        \end{minipage}
...
\begin{wrapfigure}{r}{0.45\textwidth}
    ...
    \caption{Two different sampling trajectories of decoding a length-2 sequence on a simple vocabulary $\{\texttt{A},\texttt{B}\}$. Confidence-based unmasking indicates that trajectory 1 is preferred. \jiaxin{Let's tweak the example such that the two tokens are not independent? Otherwise the order is not relevant} \sijin{Changed distribution.}}
...
Despite their empirical success, the design of confidence-based methods remains largely heuristic, and the reasons for this success are not fully understood. Although earlier literature provides some intuition that these methods plan for a good reasoning path~\citep{kim2025train} and a concurrent work establishes its convergence property~\citep{cai2026confidence}, their advantages over the uniform scheme remains theoretically unclear. This is the first question we aim to address in this paper: \emph{Is confidence unmasking provably better for the time-discretized backward process?}
